import { Event, EventModel, Service, TxStateManager } from "zkwasm-ts-server";
...
function extra(app) {
    ...
    app.get('/data/market', async (req, res) => {
        try {
    ...
    app.get('/data/player/:pid1/:pid2', async (req, res) => {
        try {
    ...
    // Get all bets
    app.get('/data/bets', async (req, res) => {
        try {
    ...
    // Get player's bets
    app.get('/data/bets/:pid1/:pid2', async (req, res) => {
        try {
            const pid1 = req.params.pid1;
            const pid2 = req.params.pid2;
...
async function bootstrap(merkleRoot) {
    return [];
}
async function batchedCallback(arg, preMerkle, postMerkle) {
    await txStateManager.moveToCommit(postMerkle);
}
async function eventCallback(arg, data) {
    if (data.length == 0) {
        return;
    }
            ...
            case EVENT_PLAYER_UPDATE:
                {
                    ...
                    await PlayerModel.findOneAndUpdate({ pid1: player.pid1, pid2: player.pid2 }, player.toObject(), { upsert: true });
